// LM REICAR YT - Status automático das lives
// O arquivo live-status.json é atualizado automaticamente pelo GitHub Actions.
...
function updateLiveGameStatus(data) {
  ...
  document.querySelectorAll('[data-game]').forEach(card => {
    ...
    const isLive = Boolean(liveGame && liveGame === game);

    card.classList.toggle('is-live', isLive);
    card.classList.toggle('is-not-live', !isLive);

    if (status) {
      status.textContent = isLive ? '🔴 AO VIVO AGORA' : 'EM BREVE';
      status.classList.toggle('is-live-status', isLive);
    }
  ...
  if (liveNotice) {
    liveNotice.classList.toggle('show', Boolean(data?.live));
    liveNotice.setAttribute('aria-hidden', data?.live ? 'false' : 'true');
  }
  if (liveTitle) {
    liveTitle.textContent = data?.live && data.title ? data.title : 'Nenhuma live acontecendo agora';
  }
}
...
async function fetchLiveStatus() {
  try {
    ...
    updateLiveGameStatus(data);
  } catch (error) {
    console.warn('Status automático indisponível:', error);
    updateLiveGameStatus(FALLBACK_STATUS);
  }
}
...
copyPix?.addEventListener("click", async () => {
  try { await navigator.clipboard.writeText(pixKey); copyPix.textContent = "CHAVE COPIADA ✓"; }
  catch { copyPix.textContent = pixKey; }
...
document.querySelectorAll(".section, .support, .next-live").forEach(el => {
...
style.textContent = `.section.visible,.support.visible,.next-live.visible{opacity:1!important;transform:translateY(0)!important}`;
